...
# def ai_kovetelmenyek_generalasa(folyamat_szoveg: str) -> tuple[str, str]:
...
#     prompt = f"""
# Az alábbi munkautasítás folyamatlépései alapján készíts két külön részt magyar nyelven:
...
# - mindkét rész felsorolás legyen
...
#     if not isinstance(minosegi_lista, list) or not isinstance(balesetvedelmi_lista, list):
#         raise ValueError("Az AI válasz szerkezete hibás.")

#     minosegi_kovetelmenyek = "\n".join(f"• {sor}" for sor in minosegi_lista)
#     balesetvedelmi_szempontok = "\n".join(f"• {sor}" for sor in balesetvedelmi_lista)

#     return minosegi_kovetelmenyek, balesetvedelmi_szempontok
...
#     minosegi_kovetelmenyek, balesetvedelmi_szempontok = ai_kovetelmenyek_generalasa(folyamat_szoveg)
